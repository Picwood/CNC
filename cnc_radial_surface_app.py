...
def export_gcode(
    ...
    approach_z = min(5.0, safe_z)
    pre_plunge_z = min(0.635, approach_z)
    ...
    def _fmt_axis(axis: str, value: float) -> str:
        return f"{axis}{_fmt_num(value)}"
    ...
    for segment in segments:
        ...
        if not np.allclose(segment[:, 2], cut_z):
            raise ValueError(
                "G-code export expects constant-Z contour segments to match the reference command style."
            )

        lines.append(f"G0 {_fmt_axis('X', float(first[0]))} {_fmt_axis('Y', float(first[1]))}")
        lines.append(_fmt_axis("Z", safe_z))
        lines.append(f"G1 {_fmt_axis('Z', approach_z)} {_fmt_feed(feed)}")
        if pre_plunge_z > cut_z + 1e-9:
            lines.append(f"{_fmt_axis('Z', pre_plunge_z)} {_fmt_feed(plunge_feed)}")
        lines.append(_fmt_axis("Z", cut_z))

        if segment.shape[0] > 1:
            second = segment[1]
            lines.append(
                f"{_fmt_axis('X', float(second[0]))} {_fmt_axis('Y', float(second[1]))} {_fmt_feed(feed)}"
            )
            for point in segment[2:]:
                lines.append(
                    f"{_fmt_axis('X', float(point[0]))} {_fmt_axis('Y', float(point[1]))}"
                )

        lines.append(f"G0 {_fmt_axis('Z', safe_z)}")

    lines.extend(["M5", "M852", "G28", "M30"])
